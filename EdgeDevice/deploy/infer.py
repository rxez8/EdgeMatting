...
isDoing = False
matting_dir = os.path.join(rootPath, "matting")
...
class Predictor:
    ...
    def run(self, imgs, trimaps=None, imgs_dir=None):
        ...
        for i in tqdm.tqdm(range(0, num, args.batch_size)):
            ...
            for j in range(args.batch_size):
                ...
                result = self._postprocess(
                    results[j], trans_info[j], trimap=trimap)
                self._save_imgs(result, imgs[i + j], imgs_dir)
    ...
    def _save_imgs(self, alpha, img_path, imgs_dir):
        ...
        rgba_save_path = os.path.join(matting_dir, name + '.png')
...
def is_jpeg_image(file_path):
    try:
        with Image.open(file_path) as img:
            is_img = img.format.lower() == 'jpeg'
            img.close()
            return is_img
    except IOError:
        return False
...
# 获取目录中时间最新的一个文件
def get_latest_file():
    directory = Path(rootPath) / "uploads"
...
def main(args):
    global predictor, isDoing
    predictor = Predictor(args)

    image_dir = os.path.join(rootPath, "uploads")
    if not os.path.exists(image_dir):
        os.makedirs(image_dir)

    decrypt_dir = os.path.join(rootPath, "decrypt")
        ...
        if not isDoing:
            try:
                ...
                if len(entries) > 0:
                    ...
                    filename = filename.replace(image_dir, '')
                    if filename[0] == '/' or filename[0] == '\\':
                        filename = filename[1:]
                    isExists = os.path.exists(os.path.join(matting_dir, filename + ".png"))
                    if isExists:
                        os.remove(os.path.join(matting_dir, filename + ".png"))
                    isExists = os.path.exists(os.path.join(decrypt_dir, file.name))
                    if isExists:
                        os.remove(os.path.join(decrypt_dir, file.name))
                    decrypt_image(os.path.join(image_dir, file.name), os.path.join(decrypt_dir, file.name),
                                  generate_key())
                    if os.path.isfile(os.path.join(decrypt_dir, file.name)):
                        isDoing = True
                        imgs_list.append(os.path.join(decrypt_dir, file.name))
                        # print(" [x] Received %r" % (data,))
                        print(" [x] Received %s %s" % (formatTime(), file.name))
                        predictor.run(imgs=imgs_list, trimaps=None, imgs_dir=decrypt_dir)

                        # 打开文件
                        with open(os.path.join(matting_dir, filename + ".png"), 'rb') as f:
                            # change 'https://cloud.saveimage.url' to your url
                            response = requests.post("https://cloud.saveimage.url", files={'file': f})
                            respdata = json.loads(response.text)
                            print(respdata)
                            if respdata['success']:
                                encrypt_image(os.path.join(matting_dir, filename + ".png"),
                                              os.path.join(matting_dir, filename + "_encrypt.png"), generate_key())
                                os.remove(os.path.join(image_dir, file.name))
                                os.remove(os.path.join(decrypt_dir, file.name))
                                f.close()
                                os.remove(os.path.join(matting_dir, filename + ".png"))
                                # 将目前等待处理照片数量保存，以便任务调度选择
                                updateRedis(len(entries) - 1)
                time.sleep(1)
